...
export default class Portfolio extends Component {
  constructor(props) {
    ...
    checkStockInPortfolio = (stockArr) => {
        let holdingsCount = stockArr.length;
        let totalInvested = 0
        let symbolArr = []
        for(let i=0; i<stockArr.length; i++){
            ...
            symbolArr.push(stockArr[i].symbol)
        }
        this.setState({
            holdingsCount,
            totalInvested,
            ...
            <View style={{backgroundColor:"#1ecb98", width:"90%", borderRadius: 25, margin:"5%", padding:"5%", paddingTop:"7%", marginTop: 0, }}>
              <View style={{justifyContent:"space-between", flexDirection:"row", flexWrap:"wrap"}}>
                <View>
                    ...
                    <Text style={{color:"#fff", fontSize: 25, textAlign:"left"}}>₹{this.state.totalInvested}</Text>
                </View>
                <View>
                    <Text style={{color:"#fff", opacity: 0.9, textAlign:"right"}}>Total Returns</Text>
                    <Text style={{color:"#fff", fontSize: 25, textAlign:"right"}}>{this.state.currentPNL-this.state.totalInvested}</Text>
                </View>
              </View>
              <View style={{justifyContent:"space-between", flexDirection:"row", flexWrap:"wrap"}}>
                <View style={{marginTop: 7}}>
                    <Text style={{color:"#fff", opacity: 0.9, textAlign:"left"}}>Current</Text>
                    <Text style={{color:"#fff", fontSize: 25, textAlign:"left"}}>₹{this.state.currentPNL}</Text>
                </View>
                <View style={{marginTop: 7}}>
                    ...
                    this.state.stocksData.map((item, key) =>(
                        <>
                            <View style={{flexDirection:"row", justifyContent:"space-between", alignItems:"center", marginLeft:"7%", marginRight:"7%"}}>
                                ...
                                <View>
                                    <Text style={{fontSize: 16, fontWeight:"bold", color:(Number(item.quantity)*Number(item.currentPrice))-(Number(item.quantity)*Number(item.averagePrice)) > 0 ? "#1dcc98" : "#da540d", marginBottom: 3}}>{(Number(item.quantity)*Number(item.currentPrice))-(Number(item.quantity)*Number(item.averagePrice))}</Text>
                                </View>
                            </View>
